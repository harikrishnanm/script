...
CREATE TYPE item_type AS ENUM ('A', 'O', 'N', 'S', 'B');
...
CREATE TABLE taxonomy_item(
  ...
  item_taxonomy_id UUID NULL,
  CONSTRAINT taxonomy_id_fk FOREIGN KEY (taxonomy_id) REFERENCES taxonomy(taxonomy_id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT arr_item_taxonomy_id_fk FOREIGN KEY (item_taxonomy_id) REFERENCES taxonomy(taxonomy_id) ON DELETE CASCADE ON UPDATE CASCADE
);
